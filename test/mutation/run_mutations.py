...
muts = [
 ...
 ('src/OriginSentinel.sol', 'if (riskEngine.weightedSourceCount() <= verifier.quorum()) f |= F_SOURCE_COVERAGE_MIN;', '', 'origin: minimum-quorum source coverage ignored'),
 ('src/OriginSentinel.sol', 'return _lineFor(s);', 'return limits.maxLine;', 'origin: policyLine misreports the policy'),
 ('src/OriginSentinel.sol', 'if (vp * BPS > verifier.price() * (BPS + limits.maxPriceGapBps)) f |= F_VAT_ABOVE_EFFECTIVE;', '', 'origin: graded Vat-vs-market check removed'),
 ('src/OriginSentinel.sol', 'VatCheck public vatCheck = VatCheck.CONSERVATIVE;', 'VatCheck public vatCheck = VatCheck.GRADED;', 'origin: default rule silently switched'),
